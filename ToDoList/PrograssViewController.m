//
//  PrograssViewController.m
//  ToDoList
//
//  Created by JETSMobileLabMini10 on 17/04/2024.
//

#import "PrograssViewController.h"
#import "Task.h"
#import "DetailsViewController.h"
@interface PrograssViewController ()
...
@implementation PrograssViewController
...
- (void)viewWillAppear:(BOOL)animated{
   ...
   // _arr = [NSKeyedUnarchiver unarchiveObjectWithData:decoded];
    ...
    for(int i=0;i<_arr.count;i++){
        if([[_arr objectAtIndex:i] state]!=1)
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(_isFiltered){
        if (indexPath.section == 0) {
            ...
            details.index=(int)[_arr indexOfObject:_lowPriorityTasks[indexPath.row]];
        }
        else if (indexPath.section == 1) {
            details.task = _mediumPriorityTasks[indexPath.row];
            details.index=(int)[_arr indexOfObject:_mediumPriorityTasks[indexPath.row]];
        }
        else {
            details.task = _highPriorityTasks[indexPath.row];
            details.index=(int)[_arr indexOfObject:_highPriorityTasks[indexPath.row]];
        }
    }
    else{
        ...
        UIAlertAction *ok=[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            if(self->_isFiltered){
                if (indexPath.section == 0)
                {
                    ...
                    [self->_userDefault setObject:data forKey:@"prog"];
